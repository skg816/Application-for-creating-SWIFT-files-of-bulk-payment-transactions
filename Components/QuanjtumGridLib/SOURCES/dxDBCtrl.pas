...
function DataVarType(AFieldType: TFieldType): Integer;
const
  varFieldType: array[ftUnknown..ftTypedBinary] of Integer = (
...
function CompareNodeId(Item1, Item2: Pointer): Integer;
begin
  if TdxDBTreeListControlNode(Item1).Id = TdxDBTreeListControlNode(Item2).Id then
    Result := 0
  else if TdxDBTreeListControlNode(Item1).Id < TdxDBTreeListControlNode(Item2).Id then
    Result := -1
  else Result := 1;
end;
...
begin
  try
    if PVariant(Item1)^ = PVariant(Item2)^ then
      Result := 0
    else if PVariant(Item1)^ < PVariant(Item2)^ then
      Result := -1
    else Result := 1;
  except
    Result := -1; // Variant Error
  end;
end;
...
begin
  ...
  begin
    ...
    Value := Null;
    LookupValue := Null;
    if (F.DataType in SimpleFields) or ((F is TStringField) and
...
begin
  ...
  begin
    if (Count = 0) or ((SelectedCount = 1) and FocusedNode.Selected) then Exit;
    BeginSelection;
    try
      ClearSelection;
...
begin
  ...
  begin
    ...
    if (VarType(Val) in [varSmallint, varInteger, varSingle, varDouble, varCurrency, varDate, varByte]) then
      Result := Val;
  end;
end;
  ...
  begin
    ...
    begin
      if (Value = GetBlankText) and (Field.Text = '') then
        ...
        Result := Field.Text = TrimRight(Value);
    end
    else
