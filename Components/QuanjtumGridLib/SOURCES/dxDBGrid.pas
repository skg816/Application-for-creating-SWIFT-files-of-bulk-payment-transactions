...
type
  ...
  public
    ...
    destructor Destroy; override;
    procedure Add(AColumn: TdxDBTreeListColumn; const AValue: Variant; const ADisplayValue: string);
    procedure AddNull(AColumn: TdxDBTreeListColumn; IsNot: Boolean);
    procedure Clear;
    function GetFilterColumnCriteria(AColumn: TdxDBTreeListColumn; var AValue: Variant): TdxDBGridFilterColumnCriteria;
    function IsColumnFilterExist(AColumn: TdxDBTreeListColumn): Boolean;
    procedure Remove(AColumn: TdxDBTreeListColumn);
    procedure RestoreDefaults;
    procedure ShowCustomDialog(AColumn: TdxDBTreeListColumn);
...
begin
  ...
  if (Button = mbLeft) and (ssDouble in Shift) and
    (State = tsNormal) and (HitInfo.hitType in NodeHitTests) and
...
begin
  Result := (Filter.FilterStatus <> fsNone) and
    ((Filter.FilterStatus = fsAlways) or (FFiltering and (Filter.Criteria.Root.Count > 0)));
end;
        ...
        begin
          with FSummaryGroup.SummaryItems[I] do
            ...
            begin
              ...
              if (SummaryType <> cstCount) and Assigned(FSmrField) and
                (FSmrField.DataType in [ftDate, ftTime, ftDateTime]) then
                DataType := FSmrField.DataType
              else
                DataType := ftUnknown;
              Result := Result + GetSummaryString(SummaryType, SummaryFormat, V, DataType, False);
              if SummaryType <> cstNone then
...
begin
  ...
  begin
    ...
    if (ANode <> Nil) then
    begin
      if (TdxDBGridNode(ANode).Id = Id) and (TdxDBGridNode(ANode[0]).Id = Id) then
      begin
        if (TdxDBGridNode(ANode[0]).Id = Id) and
           ANode[0].Focused then GotoNode(ANode[0])
        else
        begin
      ...
      for i := 0 to ANode.Count - 1 do
      try
        if (TdxDBGridNode(ANode[i]).Id = Id) then
        begin
          GotoNode(ANode[i]);
          Exit;
        end;
  ...
  begin
    for i := 0 to Self.Count - 1 do
    try
      if (TdxDBGridNode(Items[i]).Id = Id) then
      begin
        GotoNode(Items[i]);
        Exit;
      end;
...
begin
  ...
  begin
    ...
    if not ((Datalink.DataSet.RecordCount = 0) and (Datalink.DataSet.State = dsInsert)) then // TODO: Check!
      UpdateScrollBars;
...
//if NodeLinkList <> nil then NodeLinkList.Remove(ANode); // TODO CHECK
      ...
      begin
        ...
        if Assigned(APrevFocusedNode) and Assigned(ANewFocusedNode) then
        begin
...
begin
  Result := Filter.Criteria.Root.Count > 0;
end;
...
begin
  FFiltering := Filter.Criteria.Root.Count > 0;
  RefreshFilter;
  UpdateDataSetFilter;
end;
...
begin
  if FFilter = nil then
    FFilter := TdxDBGridFilter.Create(Self);
  Result := FFilter;
end;
...
begin
  ...
  begin
    I := (L + H) shr 1;
    C := CompareVariant(FSaveIdList[I], @Id);
