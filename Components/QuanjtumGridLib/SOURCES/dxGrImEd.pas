...
{       ExpressQuantumGrid image column editor                      } 
{                                                                   }
...
uses
  ...
  StdCtrls, ExtCtrls, dxCntner, Grids, Menus, dxDBTLCl;

type
TExtStringGrid = class;

TFdxGrImageValuesEdit = class(TForm)
  Panel1: TPanel;
  BOk: TButton;
  BCancel: TButton;
  Panel2: TPanel;
...
private
  FOldText: String;
  function GetForm: TFdxGrImageValuesEdit;
...
function ShowImagesValuesEditor(AColumn : TdxDBTreeListImageColumn) : Boolean;

var
  FdxGrImageValuesEdit: TFdxGrImageValuesEdit;
...
function ShowImagesValuesEditor(AColumn : TdxDBTreeListImageColumn) : Boolean;
Var
  AForm : TFdxGrImageValuesEdit;
  C, i, k : Integer;
begin
  Result := False;
  AForm := TFdxGrImageValuesEdit.Create(Nil);
  with AForm do
  begin
    ...
    begin
      if AColumn.ATreeList <> Nil then AColumn.ATreeList.BeginUpdate;
      try
        ...
        if AColumn.ATreeList <> nil then AColumn.ATreeList.EndUpdate;
      end;
      Result := True;
    end;
  end;
...
function TExtStringGrid.GetForm: TFdxGrImageValuesEdit;
begin
  Result := TFdxGrImageValuesEdit(GetParentForm(Self));
end;
...
procedure TFdxGrImageValuesEdit.BAddClick(Sender: TObject);
begin
  with StringGrid do
  begin
    RowCount := RowCount + 1;
...
procedure TFdxGrImageValuesEdit.SetImageIndexes;
var
  i: Integer;
begin
  if ADefaultImages then
...
procedure TFdxGrImageValuesEdit.BDeleteClick(Sender: TObject);
var
  i, j: Integer;
begin
  with StringGrid do
...
procedure TFdxGrImageValuesEdit.BInsertClick(Sender: TObject);
var
  i, j: Integer;
begin
  with StringGrid do
...
// protected TFdxGrImageValuesEdit
procedure TFdxGrImageValuesEdit.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
// private TFdxGrImageValuesEdit
procedure TFdxGrImageValuesEdit.SetImages(Value: TImageList);
begin
  FImages := Value;
  if Value <> nil then
...
procedure TFdxGrImageValuesEdit.WMGetMinMaxInfo(var Message : TWMGetMinMaxInfo);
begin
  Message.MinMaxInfo^.ptMinTrackSize := Point(300, 300);
  inherited;
end;

procedure TFdxGrImageValuesEdit.SetColumnWidths;
const
  col1 = 50;
  col2 = 50;
  colTotal = 50 + 50 + 100;
...
procedure TFdxGrImageValuesEdit.StringGridDrawCell(Sender: TObject; Col,
...
procedure TFdxGrImageValuesEdit.FormCreate(Sender: TObject);
begin
