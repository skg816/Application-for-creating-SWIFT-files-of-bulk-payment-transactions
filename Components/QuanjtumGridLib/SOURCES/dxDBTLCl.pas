...
begin
  if Assigned(Field) then
    if Field.IsNull and (Value = NullDate) then
      Result := True
    else  
      Result := Field.AsFloat{AsDateTime} = Value //incorrect DB.DateTimeField
  else
    Result := inherited IsEqualValues(Value);
end;
...
begin
  ...
  begin
    ...
    Result := TdxInplaceDBTreeListCheckEdit(AInplaceEdit).State;
  end
  else
    Result := inherited InitEditValue(ANode, AInplaceEdit);
end;
...
begin
  ...
  begin
    case AFieldType of
      ...
      ftCurrency: Info.ColumnClass := TdxDBTreeListCurrencyColumn;
