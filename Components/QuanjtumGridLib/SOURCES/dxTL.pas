...
{
function CustomCompareNodesDesc(Item1, Item2: Pointer): Integer;
begin
  Result := 0;
  with PNodeVariant(Item1)^.Node do
    Owner.OnCompare(Owner, PNodeVariant(Item1)^.Node, PNodeVariant(Item2)^.Node, Result);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            else W := BandWidth - HeaderLeftCoord;
...
//    if AIndex > (GetVisibleHeaderCount - 1) then AIndex := 0;
  ...
  function DoMoveFocusedColumn(FlagUp: Boolean): Boolean;
  var
...
//             FocusedNumber := FocusedNumber - RowCount;
        ...
        VK_UP: begin
            if not (IsVertThrough and DoMoveFocusedColumn(True{Up})) then
            begin
              if IsTopNode(FocusedNode) then
                NewItemRowActive := True
              else
        ...
        VK_DOWN: begin
            if not (IsVertThrough and DoMoveFocusedColumn(False{Down})) then
              DoSelection(1);
          end;
...
begin
  if (FInplaceEdit <> nil) then
    try
      ...
      if ABackFocus and Self.HandleAllocated and IsWindowVisible(Self.Handle) then
...
begin
  ...
  if (Button = mbLeft) and (Items.Count > 0) and (ItemIndex > -1) and
    (ItemAtPos(Point(X, Y), False) = ItemIndex) and (TreeList <> nil) and IsWindowVisible(TreeList.Handle) then
  begin
    FPointDragging.X := X;
    FPointDragging.Y := Y;
    FDragFlag := True;
...
begin
  Result := ACanvas.TextWidth(ANode.Strings[Index]);
end;
...
begin
  if (LockUpdate = 0) and (FHeaderBounds.Count = 0) and not FHeaderBoundsLoading then
    MakeBoundsInfo;
  if not FHeaderBoundsLoading and (FHeaderBounds.Count > 0) then // speed
...
begin
  Result := not FBands.Items[AbsoluteIndex].Visible;
end;
