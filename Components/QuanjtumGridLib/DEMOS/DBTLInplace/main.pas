...
uses
  ...
  dxCntner, dxTL, ComCtrls, Db, DBTables, Menus, dxDBTLCl,
  dxDBCtrl, dxDBTL, ExtCtrls, Buttons, Spin, StdCtrls, 
  dxExEdtr, dxCalc, dxDBEdtr, dxEdLib, dxEditor;
...
type
  TfmMain = class(TForm)
    ...
    colStandard: TdxDBTreeListMaskColumn;
    colMask: TdxDBTreeListMaskColumn;
    colCurrency: TdxDBTreeListCurrencyColumn;
    colButton: TdxDBTreeListButtonColumn;
    colDate: TdxDBTreeListDateColumn;
    colTime: TdxDBTreeListTimeColumn;
    colCheck: TdxDBTreeListCheckColumn;
    colSpin: TdxDBTreeListSpinColumn;
    colImage: TdxDBTreeListImageColumn;
    colLookup: TdxDBTreeListLookupColumn;
    colPickList: TdxDBTreeListPickColumn;
    colBlobMemo: TdxDBTreeListBlobColumn;
    colBlobPicture: TdxDBTreeListBlobColumn;
    colCalc: TdxDBTreeListCalcColumn;
    colHyperLink: TdxDBTreeListHyperLinkColumn;
    ...
    procedure dxDBTreeListChangeColumn(Sender: TObject; Node: TdxTreeListNode;
      Column: Integer);
    procedure dxDBTreeListColumnMoved(Sender: TObject; FromIndex,
      ToIndex: LongInt);
    ...
    procedure dxDBTreeListMouseUp(Sender: TObject; Button: TMouseButton;
    ...
    procedure seMaxLenChange(Sender: TObject);
    procedure edMemoMaxLengthExit(Sender: TObject);
    procedure rgPopupBorderImClick(Sender: TObject);
    ...
    procedure rgButtonStyleClick(Sender: TObject);
  ...
  private
    procedure BindColumn2TabSheet(AColumn : TdxDBTreeListColumn; ATabSheet : TTabSheet);
    function ColumnByTabSheet(ATabSheet : TTabSheet): TdxDBTreeListColumn;
    function TabSheetByColumn(AColumn : TdxDBTreeListColumn): TTabSheet;
...
begin
  ...
  BindColumn2TabSheet(TdxDBTreeListColumn(colMask), tsMask);
  BindColumn2TabSheet(TdxDBTreeListColumn(colButton), tsButton);
  BindColumn2TabSheet(TdxDBTreeListColumn(colDate), tsDate);
  BindColumn2TabSheet(TdxDBTreeListColumn(colCheck), tsCheck);
  BindColumn2TabSheet(TdxDBTreeListColumn(colImage), tsImage);
  BindColumn2TabSheet(TdxDBTreeListColumn(colSpin), tsSpin);
  BindColumn2TabSheet(TdxDBTreeListColumn(colLookup), tsLookup);
  BindColumn2TabSheet(TdxDBTreeListColumn(colPickList), tsPickList);
  BindColumn2TabSheet(TdxDBTreeListColumn(colCalc), tsCalc);
  BindColumn2TabSheet(TdxDBTreeListColumn(colBlobMemo), tsBlobMemo);
  BindColumn2TabSheet(TdxDBTreeListColumn(colBlobPicture), tsBlobPicture);
  BindColumn2TabSheet(TdxDBTreeListColumn(colCurrency), tsCurrency);
  BindColumn2TabSheet(TdxDBTreeListColumn(colTime), tsTime);
  BindColumn2TabSheet(TdxDBTreeListColumn(colHyperLink), tsHyperLink);
  // load list box
  ...
  LoadProperties;
  // init items
  miFlat.Checked := dxDBTreeList.LookAndFeel = lfFlat;
  miAutoPreview.Checked := etoPreview in dxDBTreeList.OptionsView;
end;

procedure TfmMain.BindColumn2TabSheet(AColumn: TdxDBTreeListColumn; ATabSheet: TTabSheet);
begin
...
function TfmMain.ColumnByTabSheet(ATabSheet : TTabSheet): TdxDBTreeListColumn;
begin
  Result := TdxDBTreeListColumn(ATabSheet.Tag);
end;

//returns TabsSheet tied to the given column
function TfmMain.TabSheetByColumn(AColumn : TdxDBTreeListColumn): TTabSheet;
begin
...
begin
  ...
  cbPickListSorted.Checked := colPickList.ItemsSorted;
  // Calc
...
begin
  dxDBTreeListChangeColumn(nil, nil, 0);
end;

// call refresh PageControl
procedure TfmMain.dxDBTreeListColumnMoved(Sender: TObject; FromIndex,
  ToIndex: LongInt);
begin
  dxDBTreeListChangeColumn(nil, nil, 0);
end;
...
// refresh PageControl
procedure TfmMain.dxDBTreeListChangeColumn(Sender: TObject;
  Node: TdxTreeListNode; Column: Integer);
begin
  pgColumns.ActivePage := TabSheetByColumn(TdxDBTreeListColumn(dxDBTreeList.VisibleColumns[dxDBTreeList.FocusedColumn]));
end;

// change Active Page - find Column
procedure TfmMain.pgColumnsChange(Sender: TObject);
begin
  dxDBTreeList.FocusedColumn := ColumnByTabSheet(pgColumns.ActivePage).Index;
  LoadProperties;
end;
...
begin
   ...
   begin
     ...
     BeginUpdate;
     try
       Column := TdxDBTreeListColumn(ColumnByTabSheet(pgColumns.ActivePage));
       if Column <> nil then
...
procedure TfmMain.dxDBTreeListMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  edWidth.Value := colStandard.Width;
end;
...
begin
  dxDBTreeList.ColumnsCustomizing;
end;

procedure TfmMain.miFlatClick(Sender: TObject);
begin
  dxDBTreeList.CancelEditor;
  if dxDBTreeList.LookAndFeel = lfFlat then
  begin
    dxDBTreeList.LookAndFeel := lfstandard;
    dxEditStyleController.BorderStyle := xbs3D;
    dxCheckEditStyleController.ButtonStyle := bts3D;
  end else
  begin
    dxDBTreeList.LookAndFeel := lfFlat;
...
begin
  dxDBTreeList.CancelEditor;
  if etoPreview in dxDBTreeList.OptionsView then
    dxDBTreeList.OptionsView := dxDBTreeList.OptionsView - [etoPreview]
  else dxDBTreeList.OptionsView := dxDBTreeList.OptionsView + [etoPreview];
  miAutoPreview.Checked := etoPreview in dxDBTreeList.OptionsView;
end;
...
begin
  colSpin.Increment := StrToFloat(edIncrement.Text);
end;

procedure TfmMain.edMinValueExit(Sender: TObject);
begin
  colSpin.MinValue := StrToFloat(edMinValue.Text);
end;

procedure TfmMain.edMaxValueExit(Sender: TObject);
begin
  colSpin.MaxValue := StrToFloat(edMaxValue.Text);
end;
...
procedure TfmMain.seMaxLenChange(Sender: TObject);
begin
...
procedure TfmMain.rgPopupBorderImClick(Sender: TObject);
begin
...
procedure TfmMain.rgButtonStyleClick(Sender: TObject);
begin
...
begin
  colCurrency.MinValue := StrToFloat(edMin.Text);
end;

procedure TfmMain.edMaxExit(Sender: TObject);
begin
  colCurrency.MaxValue := StrToFloat(edMax.Text);
end;
...
begin
  colPickList.ItemsSorted := cbPickListSorted.Checked;
end;
